...
epitopes <- read.table('data_table/numbering_table_unix.csv', head=T, sep=',', stringsAsFactors = F)

b.edges <- read.table('epitope_data/b_cell_nonhuman_nonlinear.edges', head=T, sep='\t')
...
pdf('analysis/b_nonhuman_nonlinear_network_99.pdf', height=7, width=7, useDingbats = F)
par(mar=c(0,0,0,0))
plot(net,
     layout=l,
     edge.width=0.1, 
