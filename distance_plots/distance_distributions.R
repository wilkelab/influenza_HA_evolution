...
df <- data.frame(distances = c(dist.epitope.99, dist.non_epitope.99, dist.epitope.rename, dist.non_epitope.rename),
                 ...
                 id = c(rep("Epitopes Bush 1999", length(dist.epitope.99)), 
                        ...
                        rep("Epitopes IEDB", length(dist.epitope.rename)), 
                        rep("Non-epitopes IEDB", length(dist.non_epitope.rename))))
...
p2 <- ggplot(df[df$id == "Epitopes IEDB" | df$id == "Non-epitopes IEDB", ], aes(x=distances, fill=factor(id))) + 
  geom_density(alpha = 0.3) + 
  scale_x_continuous(limits = c(0, 135)) +
  scale_y_continuous(limits = c(0, 0.033)) +
  scale_fill_discrete(name="") +
...
p4 <- ggplot(df[df$id == "Epitopes IEDB" | df$id == "Non-epitopes IEDB", ], aes(x=rsa, fill=factor(id))) + 
  geom_density(alpha = 0.3) + 
  scale_x_continuous(limits = c(0, 1)) +
  scale_y_continuous(limits = c(0, 3.75)) +
  scale_fill_discrete(name="") +
...
ggsave("distance_plots/distance_distributions.pdf", p, width=10, height=10)