...
p <- p + geom_tile(aes(x=index-0.01, y=rep(-0.1, length(index)), fill=id), height=0.12, width=0.75)
p <- p + scale_x_continuous(breaks=seq(0, 550, 100), limits=c(0, 550), expand=c(0,0))
p <- p + scale_y_continuous(breaks=seq(0, 5, 1), limits=c(-0.16, 5.1), expand=c(0,0))
...
ggsave(p, file='~/Google Drive/Data/influenza_HA_evolution/analysis/b_epitopes.pdf', width=10, height=5)

##T cell epitopes
binary.eps <- rep(NA, length(t.linear.eps))
binary.eps[(t.linear.eps) > 0] <- t.linear.eps[t.linear.eps > 0]
...
p <- ggplot(aes(x=index, y=binary.eps, colour=id, fill=id), data=df) + geom_bar(stat='identity', width=0.1) + 
  scale_colour_manual(values=cbbPalette) + 
  scale_fill_manual(values=cbbPalette)
p <- p + geom_tile(aes(x=index-0.01, y=rep(-0.4, length(index)), fill=id), height=0.5, width=0.75)
p <- p + scale_x_continuous(breaks=seq(0, 550, 100), limits=c(0, 550), expand=c(0,0))
p <- p + scale_y_continuous(breaks=seq(0, 25, 5), limits=c(-0.65, 25.5), expand=c(0,0))
...
ggsave(p, file='~/Google Drive/Data/influenza_HA_evolution/analysis/t_epitopes.pdf', width=10, height=5)