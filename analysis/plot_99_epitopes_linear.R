...
id <- epitopes$Bush.99[!is.na(epitopes$Protein)]
id[id=='-'] <- 'None'
...
p <- p + geom_tile(aes(x=index-0.01, y=rep(-0.6, length(index)), fill=id), height=0.6, width=0.75)
p <- p + scale_x_continuous(breaks=seq(0, 550, 100), limits=c(0, 550), expand=c(0,0))
p <- p + scale_y_continuous(breaks=seq(0, 20, 4), limits=c(-0.9, 18.1), expand=c(0,0))
...
ggsave(p, file='~/Google Drive/Data/influenza_HA_evolution/analysis/b_nonhuman_linear_99.pdf', width=10, height=5)
